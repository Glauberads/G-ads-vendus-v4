...
export function useLeadsManager() {
  ...
  const { data: leadsData, isLoading, isError, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      PerformanceMonitor.startTimer('get_admin_leads');
      ...
      if (filters.tagIds.length > 0) {
        ...
        if (restrictToIds.length === 0) {
          PerformanceMonitor.endTimer('get_admin_leads');
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: async () => {
      ...
      PerformanceMonitor.startTimer('get_leads_stats');
      ...
      PerformanceMonitor.endTimer('get_leads_stats');
