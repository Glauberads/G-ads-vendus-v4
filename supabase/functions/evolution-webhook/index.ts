...
/** Tenta adquirir lock por conversa. Retorna true se conseguiu. */
async function acquireConversationLock(
  ...
  // INSERT ... ON CONFLICT DO UPDATE WHERE expirado
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error) {
    console.warn("[anti-spam] lock acquire error:", error.message);
    return true; // fail-open: prefere responder a travar tudo
  }
  // Race: se outra invocação já estava com lock vivo, upsert sobrescreveria.
  // Defesa: cheque novamente — se locked_until anterior era no futuro e locked_by != nosso, aborte.
  // Implementação simples: se nosso lockId está lá, é nosso.
  return data?.locked_by === lockId;
}
  ...
  try {
    ...
    if (norm.kind === "message") {
      ...
      if (isFromDevice) {
        ...
        if (norm.messageId) {
          const { data: existingMsg } = await supabase
            ...
            .maybeSingle();
          if (existingMsg?.id) {
            console.log("[evolution-webhook] skip: outbound echo (external_id match)", norm.messageId);
        ...
        // Dedupe 2: mesmo conteúdo outbound nos últimos 60s para o mesmo destinatário
        if (norm.content && remotePhoneCandidates.length) {
          const since = new Date(Date.now() - 60_000).toISOString();
          const { data: recentSameContent } = await supabase
            .from("webchat_messages")
            .select("id, conversation_id, webchat_conversations!inner(visitor_phone, evolution_instance_id)")
            .eq("direction", "outbound")
            .eq("content", norm.content)
            .gte("created_at", since)
            .in("webchat_conversations.visitor_phone", remotePhoneCandidates)
            .eq("webchat_conversations.evolution_instance_id", instance.id)
            .limit(1)
            .maybeSingle();
          if ((recentSameContent as any)?.id) {
            console.log("[evolution-webhook] skip: outbound echo (recent content match)");
        ...
        if (!convOut?.id && !remotePhoneCanonical) {
          console.log("[evolution-webhook] skip: lid_no_phone (sem telefone real e sem match por LID)", { lidId });
        ...
        if (convOut?.id) {
          const { data: insertedAgentMsg } = await supabase
            .from("webchat_messages")
            .insert({
      ...
      if (groupingEnabled && groupingWindowMs > 0 && savedMessageCreatedAt) {
        ...
        const tickMs = Math.min(1000, groupingWindowMs);
        let waited = 0;
        let deferred = false;
        console.log("[evolution-webhook] buffer: window=", groupingWindowMs, "max=", groupingMaxMs);

        // Espera inicial = janela padrão. Se nenhuma msg nova → segue p/ IA.
        // Se chegou msg nova durante a espera, esta invocação sai (a nova cuida).
        while (waited < groupingWindowMs && (Date.now() - startedAt) < groupingMaxMs) {
          const slice = Math.min(tickMs, groupingWindowMs - waited);
          ...
          const { data: newer } = await supabase
            ...
            .gte("created_at", savedMessageCreatedAt)
            .neq("id", savedMessageId || "00000000-0000-0000-0000-000000000000")
            .limit(5);

          const hasNewer = (newer || []).some((m: any) => {
            if (!m) return false;
            if (m.created_at > savedMessageCreatedAt) return true;
            if (m.created_at === savedMessageCreatedAt && savedMessageId && m.id > savedMessageId) return true;
            return false;
          });
        ...
        if (deferred) {
          console.log("[evolution-webhook] buffer: defer (newer msg arrived)");
        ...
        if (conv && conv.status === "bot_active") {
          ...
          if (!productIdForBot && !orchOwnsConversation) {
            ...
            if (!botRes.ok) {
              ...
              // ============================================================
              // HARD CAP WhatsApp — máximo 1 bolha (até ~500 chars) ou 2 bolhas.
              // Defesa em profundidade: webchat-bot já faz isso, mas garantimos
              // aqui também para nunca disparar 3+ mensagens (queima de número).
              // ============================================================
              if (chunks.length > 0) {
                const totalLen = chunks.reduce((acc, c) => acc + c.length, 0);
                const MAX_BUBBLES = totalLen <= 500 ? 1 : 2;
                if (chunks.length > MAX_BUBBLES) {
                  if (MAX_BUBBLES === 1) {
                    chunks = [chunks.join("\n\n").trim()];
                    betweenDelaysMs = [];
                    typingMsPerBubble = typingMsPerBubble.length
                      ? [typingMsPerBubble.reduce((a, b) => a + b, 0)]
                      : [];
                  } else {
                    const head = chunks[0];
                    const tail = chunks.slice(1).join("\n\n").trim();
                    chunks = [head, tail];
                    betweenDelaysMs = betweenDelaysMs.length ? [betweenDelaysMs[0] ?? 1200] : [1200];
                    typingMsPerBubble = typingMsPerBubble.length >= 2
                      ? [typingMsPerBubble[0], typingMsPerBubble.slice(1).reduce((a, b) => a + b, 0)]
                      : typingMsPerBubble;
                  }
                  console.log("[evolution-webhook] whatsapp hard-cap: chunks=", chunks.length, "totalLen=", totalLen);
                }
              }
