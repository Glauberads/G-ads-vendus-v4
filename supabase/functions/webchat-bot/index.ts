...
10. Se o envio falhar por algum motivo técnico, mande pelo menos o LINK oficial do item (nunca devolva resposta vazia).`;
    ...
    if (chunkedEnabled && !responseButtons) {
      ...
      // ============================================================
      // HARD CAP por canal — anti-spam (queima de número/ferramenta)
      //   WhatsApp: 1 bolha se a resposta couber em ~500 chars,
      //             senão no máximo 2 bolhas (excesso é mesclado na última).
      //   Outros canais: mantém o comportamento original do humanizer.
      // ============================================================
      ...
      if (isWhatsApp && bubbles.length > 0) {
        const totalLen = bubbles.reduce((acc, b) => acc + b.length, 0);
        // Escala natural: textos muito curtos vão em 1 bolha; médios até 2;
        // longos até 3. Nunca passa de 3 (anti-spam / queima de número).
        const MAX_BUBBLES = totalLen <= 120 ? 1 : (totalLen <= 320 ? 2 : 3);
        if (bubbles.length > MAX_BUBBLES) {
          if (MAX_BUBBLES === 1) {
            bubbles = [bubbles.join('\n\n').trim()];
            between = [];
            typingMs = typingMs.length ? [typingMs.reduce((a, b) => a + b, 0)] : [];
          } else {
            // Mantém as primeiras (MAX_BUBBLES - 1) e mescla o resto na última.
            const head = bubbles.slice(0, MAX_BUBBLES - 1);
            const tail = bubbles.slice(MAX_BUBBLES - 1).join('\n\n').trim();
            bubbles = [...head, tail];
            // Ajusta between/typing para o novo número de bolhas
            between = between.slice(0, MAX_BUBBLES - 1);
          ...
          console.log('[webchat-bot] whatsapp cap: reduced bubbles to', bubbles.length, 'totalLen=', totalLen);
        }
      }
